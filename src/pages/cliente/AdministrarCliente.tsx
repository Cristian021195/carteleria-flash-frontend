...
export const AdministrarCliente = () => {
  ...
  const exportar = async () => {
    if(validate()){
      try {
        setLoading(true);
        const pet = await fetch(url+"/export/cliente", {
          method:'POST',
          body:JSON.stringify(fData),//, credentials:'include'
          headers:{
            'Content-Type':'application/json'
  ...
  return (
    <div className="fade-up max-w-[80vw] mx-auto">
      <div className="bg-[#222E3C] text-white">
        <h1 className="text-2xl p-2">ADMINISTRAR CLIENTES</h1>
      </div>
              ...
              {hader:'TELEFONO2',accessorKey:'telefono2'},
              {
                ...
                cell: (element:any) => (
                  <div>
                    ...
                    &emsp;
                    <Link to={`/cliente/${element.row.original.id}/editar`} className="p-1 rounded text-white bg-slate-300">
                      ✏️
                    </Link>
                  </div>
                ),
